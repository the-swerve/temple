...
var Environment = module.exports = function(parent_node) {
	...
	while (stack.length > 0) {
		...
		if (loop) {
			...
			var cond = current_node.getAttribute(prefix + 'visible')
			if (cond) {
				current_node.removeAttribute(prefix + 'visible');
				this.interpolations.push(new Conditional(current_node, cond.replace(/\s+/g,'')));
			}
			// Find all other deja attributes that aren't texts or conditionals
