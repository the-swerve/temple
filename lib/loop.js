...
var Loop = module.exports = function(node, props) {
	this.node = node;
...
Loop.prototype.render = function(model) {
	...
	// nodes to be resorted to reflect this rather than changing the data in the
	// existing DOM node order.
	// eg: <i data-checked>1</i>, <i>2</i>, <i>3</i>
	// user reorders array, and we should get:
	...
	for (var i = 0; i < arr.length; ++i) {
		...
		if (existing) {
			...
			var new_env = new Environment(new_node);
